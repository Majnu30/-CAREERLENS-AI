...
def generate_report_markdown(analysis, target_role, roadmap_steps, match_res=None) -> str:
    name = analysis.get("name", "Candidate") if analysis else "Candidate"
    score = analysis.get("resume_score", "N/A") if analysis else "N/A"
    skills = ", ".join(analysis.get("skills", [])) if analysis else "None listed"
    
    report = f"""# CAREER AUDIT REPORT
**Candidate:** {name}
**Generated Date:** {datetime.now().strftime('%B %d, %Y')}
**Resume Match Index:** {score}/100

---
### 1. CORE SKILLS IDENTIFIED
{skills}

---
### 2. TARGET CAREER PATH
**Goal:** {target_role}
...
"""
    if roadmap_steps:
        ...
        report += "• Complete skill alignment review\n• Expand technical project portfolio\n• Refine resume formatting\n"
        
    if match_res:
        report += f"\n---\n### 4. TARGET JOB MATCH STATS\n"
        report += f"• Overall Role Fit: {match_res.get('overall', 0)}%\n"
        report += f"• Matched Competencies: {', '.join(match_res.get('matched', []))}\n"
        report += f"• Recommended Skills to Add: {', '.join(match_res.get('missing', []))}\n"
...
def open_register_dialog():
    st.markdown("Create an account to save your resume score and career roadmaps.")
    ...
    with col_l2:
        st.markdown(
            ...
            <div class="panel" style="padding: 30px; text-align: center;">
                ...
                <h3 style="margin: 8px 0 0 0; color: #f4f7fb;">Analyze. Optimize. Accelerate.</h3>
                <p style="color: #94a3b8; font-size: 0.92rem; margin-top: 6px; margin-bottom: 22px;">
                    Choose an option below to start reviewing your resume, salary insights, and career roadmaps.
                </p>
            </div>
            """,
            unsafe_allow_html=True,
...
if st.session_state.workspace == "Job Seeker":
    ...
    with tabs[0]:
        ...
        if st.session_state.resume_analysis:
            ...
            st.markdown("---")
            st.markdown("#### 📥 Download Executive Career Audit Report")
            report_text = generate_report_markdown(
                res,
                st.session_state.custom_action_plan.get("role", "Target Industry Role") if st.session_state.custom_action_plan else "Target Industry Role",
                st.session_state.custom_action_plan.get("steps", []) if st.session_state.custom_action_plan else [],
                st.session_state.get("current_job_match")
            )
            st.download_button(
                "⬇️ Download Summary Report (.md / PDF Ready)",
                report_text.encode("utf-8"),
                file_name=f"{res.get('name', 'Candidate')}_Career_Audit.md",
                mime="text/markdown",
...
# ============================================================
    ...
    st.markdown(
        ...
        <section class="hero">
            ...
            <h1>Platform Analytics.<br><span>User Audit & Ratings.</span></h1>
