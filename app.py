...
with st.sidebar:
    ...
    try:
        ...
        if health_check.status_code == 200:
            st.markdown('<span class="status">● BACKEND CONNECTED</span>', unsafe_allow_html=True)
        else:
...
if workspace == "👨‍💻 Job Seeker":
    ...
    with tabs[0]:
        ...
        if resume_file and st.button("Analyze Resume via API", type="primary", use_container_width=True):
            with st.spinner("Connecting to Render backend..."):
                try:
                    ...
                    st.success("Resume processed by CareerLens Backend!")
        ...
        if st.session_state.resume_analysis:
            ...
            with c2:
                ...
                st.write("**Experience:**", res.get("experience", "Detected"))
    ...
    with tabs[1]:
        ...
        if st.button("Analyze Match via API", type="primary", use_container_width=True):
    ...
    with tabs[2]:
        ...
        if st.button("Run Risk Analysis via API", type="primary", use_container_width=True):
    ...
    with tabs[3]:
        ...
        if st.button("Analyze Skill Gap via API", type="primary", use_container_width=True):
            if not st.session_state.resume_text:
...
# ============================================================
    ...
    recruiter_job = st.text_area("Job Description", height=200, key="recruiter_job")
    ...
    if st.button("🚀 Screen Candidates via Backend API", type="primary", use_container_width=True):
